              ...
              <tbody>
                {filteredEmployees.map((emp) => (
                  <tr key={emp._id} className="border-b hover:bg-slate-50/50 transition">
                    ...
                    <td className="p-4">
                      <span className="flex items-center gap-1.5 text-slate-500"><Mail size={14} /> {emp?.userId?.email}</span>
                    </td>
                    <td className="p-4">
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl space-y-6">
            ...
            <form onSubmit={handleUpdateSubmit} className="space-y-4 max-h-[75vh] overflow-y-auto pr-1">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5">Reset Password (Leave blank to keep current)</label>
                <input
                  type="password"
