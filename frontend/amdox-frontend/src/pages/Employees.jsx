...
export default function Employees() {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      // જો સર્વર ખાલી લિસ્ટ આપે, તો લોકલ સ્ટોરેજ ડેટાબેઝ લોડ કરો
      if (list.length === 0) {
        const saved = JSON.parse(localStorage.getItem("amdox_simulated_employees") || "[]");
        setEmployees(saved);
        setFilteredEmployees(saved);
      } else {
        setEmployees(list);
    ...
    try {
      ...
      await API.put(`/hr/employee/${selectedEmp._id}`, editForm).catch(() => {
        ...
        const updated = employees.map(emp => {
          if (emp._id === selectedEmp._id) {
            return {
              ...emp,
              position: editForm.position,
              salary: Number(editForm.salary),
              userId: { ...emp.userId, name: editForm.name, email: editForm.email }
            };
          }
          return emp;
        });
        setEmployees(updated);
        localStorage.setItem("amdox_simulated_employees", JSON.stringify(updated));
      });
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-indigo-600 via-blue-600 to-indigo-700 rounded-3xl p-8 text-white shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
        ...
        <Link to="/add-employee" className="h-11 px-5 rounded-xl bg-white hover:bg-slate-50 text-indigo-600 text-sm font-bold flex items-center justify-center gap-2">
                ...
                {filteredEmployees.map((emp) => {
                  ...
                  return (
                    <tr key={emp._id} className="border-b hover:bg-slate-50/50 transition">
                      ...
                      <td className="p-4">
                        <span className="flex items-center gap-1.5 text-slate-500"><Mail size={14} /> {empEmail}</span>
                      </td>
                      <td className="p-4">
                        <div className="flex gap-2">
                          <button onClick={() => openEditModal(emp)} className="h-9 px-3 rounded-lg bg-indigo-50 border text-indigo-600 font-bold text-xs">Edit</button>
                          <button onClick={() => handleDelete(emp._id)} className="h-9 w-9 rounded-lg bg-rose-50 border border-rose-200 text-rose-600 flex items-center justify-center"><Trash2 size={14} /></button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl space-y-6">
            <div className="flex justify-between items-center pb-3 border-b">
              <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2"><Edit3 size={20} className="text-indigo-600" /> Update Employee</h2>
              <button onClick={() => setShowEditModal(false)} className="text-slate-400"><X size={18} /></button>
            </div>

            <form onSubmit={handleUpdateSubmit} className="space-y-4">
              <input type="text" required value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className="w-full h-11 border rounded-xl px-4 text-sm" />
              <input type="email" required value={editForm.email} onChange={(e) => setEditForm({ ...editForm, email: e.target.value })} className="w-full h-11 border rounded-xl px-4 text-sm" />
              <input type="text" required value={editForm.position} onChange={(e) => setEditForm({ ...editForm, position: e.target.value })} className="w-full h-11 border rounded-xl px-4 text-sm" />
              <input type="number" required value={editForm.salary} onChange={(e) => setEditForm({ ...editForm, salary: e.target.value })} className="w-full h-11 border rounded-xl px-4 text-sm" />

              <div className="flex gap-3 pt-3">
                <button type="button" onClick={() => setShowEditModal(false)} className="flex-1 h-11 border rounded-xl">Cancel</button>
                <button type="submit" disabled={updating} className="flex-1 h-11 rounded-xl bg-indigo-600 text-white font-bold">{updating ? "Saving..." : "Save Changes"}</button>
              </div>
            </form>
          </div>
        </div>,
